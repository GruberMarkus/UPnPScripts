option explicit
dim URL, strXML, xmlhttp, RequestHeaderContentType, RequestHeaderSOAPAction, proxy, response, Mute
...
'"Device Spy.exe" starten, ein Gerät auswählen, darunter ein Service mit "SetMute" finden
...
RequestHeaderSOAPAction = """urn:schemas-upnp-org:service:RenderingControl:1#SetMute"""
...
strXML = "<?xml version=""1.0"" encoding=""utf-8""?>" & vbcrlf & _
	...
	"	<s:Body>" & vbcrlf & _
	"		<u:SetMute xmlns:u=""urn:schemas-upnp-org:service:RenderingControl:1"">" & vbcrlf & _
	"			<InstanceID>0</InstanceID>" & vbcrlf & _
	"			<Channel>Master</Channel>" & vbcrlf & _
	"			<DesiredMute>" & Mute & "</DesiredMute>" & vbcrlf & _
	"		</u:SetMute>" & vbcrlf & _
	"	</s:Body>" & vbcrlf & _
	"</s:Envelope>"
