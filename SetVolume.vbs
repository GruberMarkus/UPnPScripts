option explicit
dim URL, strXML, xmlhttp, RequestHeaderContentType, RequestHeaderSOAPAction, proxy, response, Lautstaerke
...
'"Device Spy.exe" starten, ein Gerät auswählen, darunter ein Service mit "SetVolume" finden
...
URL = "http://192.168.1.103:2869/upnphost/udhisapi.dll?control=uuid:de86cf12-71e0-49da-9b09-3b85c791e300+urn:upnp-org:serviceId:RenderingControl"
...
'Help, Show Debug Information, Events, Show Information Messages
'Im Hauptfenster: Rechtsklick auf die gewünschte Aktion, Invoke Action, Parameter angeben, Invoke
'Im Debugfenster: Aus dem HTTP POST die den Content Type und die SOAP Action extrahieren
RequestHeaderContentType = "text/xml; charset=""utf-8"""
RequestHeaderSOAPAction = """urn:schemas-upnp-org:service:RenderingControl:1#SetVolume"""


'Lautstaerke von 0-100 festlegen
Lautstaerke = 100
...
strXML = "<?xml version=""1.0"" encoding=""utf-8""?>" & vbcrlf & _
	...
	"	<s:Body>" & vbcrlf & _
	"		<u:SetVolume xmlns:u=""urn:schemas-upnp-org:service:RenderingControl:1"">" & vbcrlf & _
	"			<InstanceID>0</InstanceID>" & vbcrlf & _
	"			<Channel>Master</Channel>" & vbcrlf & _
	"			<DesiredVolume>" & Lautstaerke & "</DesiredVolume>" & vbcrlf & _
	"		</u:SetVolume>" & vbcrlf & _
	"	</s:Body>" & vbcrlf & _
	"</s:Envelope>"
