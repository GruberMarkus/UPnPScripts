option explicit
dim URL, strXML, xmlhttp, RequestHeaderContentType, RequestHeaderSOAPAction, proxy, response, Lied
...
'"Device Spy.exe" starten, ein Gerät auswählen, darunter ein Service mit "SetAVTransportURI" finden
...
RequestHeaderSOAPAction = """urn:schemas-upnp-org:service:AVTransport:1#SetAVTransportURI"""


'Lied festlegen
'Siehe Datei _lesen.txt, Abschnitt "Ich habe einen UPnP/DLAN Media Server laufen - wie finde ich die URL einer Datei heraus?"
Lied = "http://www.tonycuffe.com/mp3/tail%20toddle.mp3"
...
strXML = "<?xml version=""1.0"" encoding=""utf-8""?>" & vbcrlf & _
	...
	"	<s:Body>" & vbcrlf & _
	"		<u:SetAVTransportURI xmlns:u=""urn:schemas-upnp-org:service:AVTransport:1"">" & vbcrlf & _
	"			<InstanceID>0</InstanceID>" & vbcrlf & _
	"			<CurrentURI>" & Lied & "</CurrentURI>" & vbcrlf & _
	"			<CurrentURIMetaData/>" & vbcrlf & _
	"		</u:SetAVTransportURI>" & vbcrlf & _
	"	</s:Body>" & vbcrlf & _
	"</s:Envelope>"
